...
export type Asset = { id: string; originalName: string | null; relativePath: string; mimeType: string; sizeBytes: number; sha256: string; createdAt: number; assetType: "image" | "file"; contentRecordId: string | null };

export function createAssetService(sqlite: Database.Database, dataDir: string) {
  const select = `SELECT id, original_name as originalName, relative_path as relativePath, mime_type as mimeType, size_bytes as sizeBytes, sha256, created_at as createdAt, asset_type as assetType, content_record_id as contentRecordId FROM assets`;
  const persist = (input: { originalName: string; mimeType: string; relativePath: string; storedName: string; sha256: string; sizeBytes: number; conversationId?: string; messageId?: string; contentRecordId?: string; ownerOperatorId?: string; source?: "upload" | "runtime" }): Asset => {
    const id = randomUUID(); const now = Date.now();
    sqlite.prepare(`INSERT INTO assets (id, conversation_id, message_id, content_record_id, owner_operator_id, asset_type, source, original_name, stored_name, relative_path, mime_type, size_bytes, sha256, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, input.conversationId ?? null, input.messageId ?? null, input.contentRecordId ?? null, input.ownerOperatorId ?? null, input.mimeType.startsWith("image/") ? "image" : "file", input.source ?? "upload", input.originalName, input.storedName, input.relativePath, input.mimeType, input.sizeBytes, input.sha256, now);
    return { id, originalName: input.originalName, relativePath: input.relativePath, mimeType: input.mimeType, sizeBytes: input.sizeBytes, sha256: input.sha256, createdAt: now, assetType: input.mimeType.startsWith("image/") ? "image" : "file", contentRecordId: input.contentRecordId ?? null };
  };
  ...
  return { get(id: string) { return sqlite.prepare(`${select} WHERE id = ?`).get(id) as Asset | undefined; }, getAuthorized(id: string, operatorId: string) { return sqlite.prepare(`${select} WHERE id = ? AND (owner_operator_id = ? OR conversation_id IN (SELECT id FROM conversations WHERE owner_operator_id = ?))`).get(id, operatorId, operatorId) as Asset | undefined; }, getViewAuthorized(id: string, operatorId: string) { return sqlite.prepare(`${select} WHERE id = ? AND (owner_operator_id = ? OR conversation_id IN (SELECT id FROM conversations WHERE owner_operator_id = ?) OR content_record_id IN (SELECT id FROM content_records WHERE created_by_operator_id = ?))`).get(id, operatorId, operatorId, operatorId) as Asset | undefined; }, listForContent(contentRecordId: string, operatorId: string) { return sqlite.prepare(`${select} WHERE content_record_id = ? AND asset_type = 'image' AND content_record_id IN (SELECT id FROM content_records WHERE created_by_operator_id = ?) ORDER BY created_at DESC`).all(contentRecordId, operatorId) as Asset[]; }, async archiveUpload(input: { originalName: string; mimeType: string; content: Buffer; conversationId?: string; messageId?: string; contentRecordId?: string; ownerOperatorId?: string; source?: "upload" | "runtime" }): Promise<Asset> {
  ...
  }, async archiveUploadStream(input: { originalName: string; mimeType: string; sizeBytes: number; stream: ReadableStream<Uint8Array>; contentRecordId?: string; ownerOperatorId?: string }): Promise<Asset> {
